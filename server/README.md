...
## Ports
- `127.0.0.1:33389`: Kotlin local control server (entry point used by app/UI and adb port-forwarding).
- `127.0.0.1:8776`: Python worker server.
- `/brain/*` routes are exposed on `:33389` and proxied to worker `:8776`.
...
## SSHD
SSHD is managed by Kotlin control plane APIs on `:33389` (not by Python worker).
...
curl -sS -X POST http://127.0.0.1:33389/brain/start
curl -sS -X POST http://127.0.0.1:33389/brain/inbox/chat \
  -H 'Content-Type: application/json' \
  -d '{"text":"hello brain"}'
curl -sS 'http://127.0.0.1:33389/brain/messages?limit=20'
```
