...
from tools.device_api import DeviceApiTool
...
class BrainRuntime:
    ...
    def _responses_tools(self) -> List[Dict[str, Any]]:
        device_api_actions = self._device_api_allowed_actions()
        return [
            {
                "type": "function",
                "name": "list_dir",
    ...
    def _device_api_allowed_actions(self) -> List[str]:
        actions = set(DeviceApiTool._ACTIONS.keys())
        actions.update(
            {
                "uvc.ptz.get_abs",
                "uvc.ptz.get_limits",
