...
tasks.named("preBuild") {
    dependsOn(syncGoogleServicesJson, syncServerAssets, syncSystemAssets, syncUserDefaults, verifyPythonRuntime, syncDependencyInventoryAsset)
}

tasks.matching { task ->
    task.name.startsWith("merge") && task.name.endsWith("NativeLibs")
}.configureEach {
    dependsOn(buildUsbLibs)
}

tasks.matching { task ->
    task.name.startsWith("assemble") ||
        task.name.startsWith("install") ||
        Regex("^bundle(?!.*Classes).+$").matches(task.name) ||
        (Regex("^package(?!.*Resources).+$").matches(task.name) && !task.name.contains("UnitTest"))
}.configureEach {
    dependsOn(fetchNodeRuntime, fetchTermuxTools)
}
