...
val buildUsbLibs by tasks.registering(Exec::class) {
    ...
    commandLine(
        "bash",
        "-lc",
        "./scripts/build_dropbear.sh && ./scripts/build_libusb_android.sh && ./scripts/build_libuvc_android.sh && ./scripts/build_methingssh_android.sh && ./scripts/build_methingspy_android.sh && ./scripts/build_methingsrun_android.sh"
    )
}


val fetchTermuxTools by tasks.registering(Exec::class) {
    val repoRoot = rootProject.projectDir.parentFile.parentFile
    workingDir = repoRoot
    // Opt-in: fetches bash, jq, ripgrep from Termux and stages them into jniLibs/assets.
    // Enable with: METHINGS_FETCH_TERMUX_TOOLS=1
    val enabled = (System.getenv("METHINGS_FETCH_TERMUX_TOOLS") ?: "").trim() == "1"
    commandLine(
        "bash",
        "-lc",
        if (enabled) "./scripts/fetch_termux_tools_android.sh" else "echo 'fetchTermuxTools: disabled (set METHINGS_FETCH_TERMUX_TOOLS=1 to enable)'"
    )
}
...
val buildArduinoCliRuntime by tasks.registering(Exec::class) {
    val repoRoot = rootProject.projectDir.parentFile.parentFile
    workingDir = repoRoot
    // Enabled by default for local builds. CI keeps this off unless explicitly enabled.
    val forceEnable = (System.getenv("METHINGS_BUILD_ARDUINO_CLI") ?: "").trim() == "1"
    val forceDisable = (System.getenv("METHINGS_BUILD_ARDUINO_CLI") ?: "").trim() == "0"
    val enabled = when {
        forceEnable -> true
        forceDisable -> false
        isCiBuild -> false
    ...
    commandLine(
        "bash",
        "-lc",
        if (enabled) "./scripts/build_arduino_cli_android.sh" else "echo 'buildArduinoCliRuntime: disabled (set METHINGS_BUILD_ARDUINO_CLI=1 to enable in CI)'"
    )
}

val buildArduinoBuiltinToolsRuntime by tasks.registering(Exec::class) {
    val repoRoot = rootProject.projectDir.parentFile.parentFile
    workingDir = repoRoot
    // Enabled by default for local builds. CI keeps this off unless explicitly enabled.
    val forceEnable = (System.getenv("METHINGS_BUILD_ARDUINO_BUILTIN_TOOLS") ?: "").trim() == "1"
    val forceDisable = (System.getenv("METHINGS_BUILD_ARDUINO_BUILTIN_TOOLS") ?: "").trim() == "0"
    val enabled = when {
        forceEnable -> true
        forceDisable -> false
        isCiBuild -> false
    ...
    commandLine(
        "bash",
        "-lc",
        if (enabled) "./scripts/build_arduino_builtin_tools_android.sh" else "echo 'buildArduinoBuiltinToolsRuntime: disabled (set METHINGS_BUILD_ARDUINO_BUILTIN_TOOLS=1 to enable in CI)'"
    )
}
...
tasks.named("preBuild") {
    dependsOn(syncGoogleServicesJson, syncServerAssets, syncSystemAssets, syncUserDefaults, verifyPythonRuntime, buildUsbLibs, fetchNodeRuntime, fetchTermuxTools, buildArduinoCliRuntime, buildArduinoBuiltinToolsRuntime, syncDependencyInventoryAsset)
}
...
tasks.named<Delete>("clean") {
    ...
    delete(
        fileTree(jniRoot) {
            ...
            include("**/libmethingsrun.so")
            include("**/libarduino-cli.so")
            include("**/libserial-discovery.so")
            include("**/libmdns-discovery.so")
            include("**/libserial-monitor.so")
        }
    )
