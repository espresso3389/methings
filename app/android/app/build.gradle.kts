...
val buildUsbLibs by tasks.registering(Exec::class) {
    ...
    commandLine(
        "bash",
        "-lc",
        "./scripts/build_libusb_android.sh && ./scripts/build_libuvc_android.sh"
    )
}
...
tasks.named("preBuild") {
    dependsOn(syncGoogleServicesJson, syncServerAssets, syncSystemAssets, syncUserDefaults, buildUsbLibs, fetchOpenCvAndroidSdk, fetchNodeRuntime, syncDependencyInventoryAsset)
}
