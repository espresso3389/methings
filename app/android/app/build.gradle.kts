...
tasks.named("preBuild") {
    dependsOn(syncGoogleServicesJson, syncServerAssets, syncSystemAssets, syncUserDefaults, verifyPythonRuntime, buildUsbLibs, buildFacadeWheels, fetchOpenCvAndroidSdk, fetchNodeRuntime, syncDependencyInventoryAsset)
}
...
tasks.named<Delete>("clean") {
    ...
    delete(
        fileTree(jniRoot) {
            include("**/libdbclient.so")
            include("**/libdropbear.so")
            include("**/libdropbearkey.so")
            include("**/libscp.so")
            include("**/libusb*.so")
            include("**/libuvc.so")
            include("**/libmethings*.so")
            // Optional Termux Node.js runtime (if staged).
            include("**/libnode.so")
            include("**/libssl.so.3")
            include("**/libcrypto.so.3")
            include("**/libcares.so")
            include("**/libz.so.1")
            include("**/libicudata.so.78")
            include("**/libicudata.so.78.*")
            include("**/libicui18n.so.78")
            include("**/libicui18n.so.78.*")
            include("**/libicuuc.so.78")
            include("**/libicuuc.so.78.*")
        }
    )
